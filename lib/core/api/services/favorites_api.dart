...
abstract class FavoritesApi {
  ...
  Future<Map<String, dynamic>> toggle(@Path('movieId') int movieId);

  /// 查询单个影片的收藏状态。
  @GET('/favorites/{movieId}/status')
  Future<Map<String, dynamic>> status(@Path('movieId') int movieId);

  /// 批量添加收藏。
  @POST('/favorites')
  Future<Map<String, dynamic>> addBatch(@Body() Map<String, dynamic> body);

  /// 批量删除收藏。
  @POST('/favorites/batch-delete')
  Future<Map<String, dynamic>> removeBatch(@Body() Map<String, dynamic> body);
}
