...
/// （或下一待看集）开始播放。
class EmbySeriesDetailPage extends ConsumerStatefulWidget {
  const EmbySeriesDetailPage({super.key, required this.seriesId});

  final String seriesId;

  @override
  ConsumerState<EmbySeriesDetailPage> createState() =>
      _EmbySeriesDetailPageState();
}

class _EmbySeriesDetailPageState extends ConsumerState<EmbySeriesDetailPage> {
  ...
  void _invalidateDetail() {
    ...
    ref.invalidate(
      embyItemDetailProvider(
        EmbyItemDetailRequest(serverId: serverId, itemId: _seriesId),
      ),
    );
  }
...
class _SeasonSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final seasons = ref.watch(
      embySeasonsProvider(EmbySeasonsRequest(serverId: serverId, seriesId: seriesId)),
    );
    final colors = appColors(context);
    return seasons.when(
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final episodes = ref.watch(
      embyEpisodesProvider(
        EmbyEpisodesRequest(serverId: serverId, seasonId: seasonId),
      ),
    );
    final urls = ref.watch(embyServerUrlsProvider);
    final colors = appColors(context);
