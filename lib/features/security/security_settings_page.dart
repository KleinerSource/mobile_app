    ...
    if (action == _CredentialAction.configure) {
      ...
      await _clearCredential(
        ...
        onConfirm: () => ref
            ...
            .clearPin(
              biometricEnabledMessage:
                  l.securityClearPinRequiresBiometricDisabled,
            ),
      );
    }
  }
      ...
      if (value) {
        ...
        if (!pinConfigured) {
          ...
          if (!pinConfigured) {
            ScaffoldMessenger.of(
              context,
            ).showSnackBar(SnackBar(content: Text(l.securitySetPinFirst)));
            return;
          }
        }
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text(l.securityBiometricDisabled)));
        }
      }
    } catch (error) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.securityBiometricUpdateFailed(error.toString())),
          ),
        );
      }
    }
      ...
      child: Column(
        ...
        children: [
          ...
          ListTile(
            ...
            title: Text(l.commonClear, style: TextStyle(color: colors.danger)),
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text(l.securityLoadFailed(message), textAlign: TextAlign.center),
