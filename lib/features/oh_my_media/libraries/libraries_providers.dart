...
final librariesRepositoryProvider = Provider<MediaLibraryRepository>((ref) {
  final registry = ref.watch(mediaSourceRegistryProvider);
  return MediaLibraryRepository(
    management: registry.requireCapability<LibraryManagementSource>(
      const SourceId('omm'),
    ),
    scanning: registry.requireCapability<ScanSource>(const SourceId('omm')),
    batchScanning: registry.requireCapability<BatchScanSource>(
      const SourceId('omm'),
    ),
  );
});
