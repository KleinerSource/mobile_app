...
## 当前任务：修复 KSPlayer 连续切换清晰度失效（2026-08-26）

### 目标

调整共享播放器的切源清理顺序，使 KSPlayer 在转码档之间、转码切原生/自动时能立即进入加载状态，并在保留播放位置和播放意图的前提下完成连续切源；libmpv 行为不得回归。

### 阶段

- [x] 恢复上下文并确认工作区基线
- [x] 使用 CodeGraph 复核切源调用链和影响范围
- [x] 调整 `PlayerPage._loadInternal` 的加载反馈与清理顺序
- [x] 补充统一播放器 `open → stop → open` 契约测试
- [x] 完成格式化、定向测试、全量测试与静态分析

### 约束

- 只修改共享切源编排和契约测试，不改 KSPlayer 原生 `open`、内核选择、公共 API、模型或服务端协议。
- 清晰度切换仍等待旧服务端转码会话清理完成，避免旧 DELETE 与新会话竞争。
- 每个异步清理边界后校验 `mounted` 与 `_loadGeneration`，失效任务不得打开旧选择。
- 退出播放器继续沿用非阻塞服务端清理策略。
...
| 错误 | 处理 |
| --- | --- |
| 首次尝试同时追加三个规划文件时，`findings.md` 使用了不存在的章节锚点，导致整个补丁未应用 | 已先读取各文件真实末尾，改为按文件使用实际 EOF 锚点分别追加 |

### 验证结果

- 播放器契约、播放路由、菜单一致性和播放模型定向测试共 27 项通过。
...
## 当前任务：修复 KSPlayer 任意第二次切源必定无限加载（2026-08-26 再续查）

### 目标

以用户给出的稳定复现规律为准，定位第一次成功切源后遗留的会话状态，确保 KSPlayer 连续任意质量切换均能完成或明确失败，不能永久停留在“正在加载影片...”。

### 阶段

- [x] 恢复上一轮排查、提交和验证状态
- [x] 还原第二次切源的原生 open/stop 完成契约与独有状态
- [x] 构造连续两次切源的可重复回归测试
- [x] 实施最小生命周期修复
- [x] 运行定向测试、静态分析、全量测试和差异检查

### 约束与成功标准

- 以“首次切换成功、第二次目标档位无关且必卡”为核心证据，不再假设某个编码或单一质量路由是根因。
- 连续 `open → stop → open → stop → open` 必须全部有界完成，并保留续播位置与播放意图。
- 不修改 libmpv 行为、服务端协议或公共播放模型。
- iOS 真机最终验证 `自动 → 720p → 480p`、`自动 → 原生 → 自动`，播放与暂停状态各一次。

### 验证结果

- 定向播放器契约与路由测试 20 项通过。
- `flutter analyze --no-pub` 通过。
- `flutter test --no-pub` 全量 421 项通过。
- `git diff --check` 通过。
- Windows 无 Swift/Xcode 工具链，原生编译与 iOS 真机矩阵仍需在 macOS/iOS 上验证。

### 错误记录

| 错误 | 处理 |
| --- | --- |
| 首次尝试同时追加三个规划文件时，`findings.md` 的末行锚点与实际文件不一致，整个补丁未应用 | 改为使用各文件当前真实末行分别追加，不重复组合补丁 |

## 当前任务：修复 KSPlayer 二次切换后无限加载（2026-08-26）

### 目标

定位第二次质量切换卡住的具体异步边界，消除无限 loading，同时保留本地播放器优先断源、旧服务端会话清理和续播行为。

### 阶段

- [x] 恢复当前代码与上一轮修改状态
- [x] 复核转码事件流取消、服务端停止和新源打开的生命周期
- [x] 用悬挂 SSE 测试排除错误假设，并确认现有非零续播契约
- [x] 实施最小修复并完成定向验证
- [x] 完成静态分析、全量测试和最终差异检查

### 约束

- 不回退“立即显示 loading、先停本地播放器”的已确认体验修复。
- 不以无界等待换取会话清理；也不能让迟到的旧 DELETE 杀掉新转码会话。
- 只修改证据指向的清理边界，不扩展公共 API 或重写 KSPlayer 原生层。

### 错误记录

| 错误 | 处理 |
| --- | --- |
| 用悬挂的 SSE `onCancel` 编写回归测试后，当前实现已能在 200ms 内完成订阅取消，测试未复现故障 | 删除无效测试，不继续修改 SSE；转向检查第二次 KSPlayer `open` 的非零续播 seek 完成条件 |
| Windows 环境没有 `swift` 或 `swift-format` | 不伪造原生编译结果；执行 Dart 契约/路由回归、静态分析和全量测试，Swift 编译与真机路径留给 iOS CI/设备验收 |
| 一次多文件补丁在 Swift 注释 hunk 后多写了空 `@@`，补丁校验失败且未应用 | 不重复该补丁；改为按 Swift 与规划文件分别使用精确 hunk 更新 |

### 验证结果

- 播放器契约、路由、菜单一致性和播放模型定向测试共 27 项通过。
- `flutter analyze --no-pub` 通过，无静态分析问题。
- `flutter test --no-pub` 全量 419 项通过。
