...
void main() {
  ...
  testWidgets('额外预览图接口成功后只刷新预览图', (tester) async {
  ...
  testWidgets('额外预览图接口失败时显示原因且不刷新预览图', (tester) async {
    final repository = _FakeMediaRepository(
      downloadError: const SourceException('DB Online 未返回预览图'),
    );
    var loadCount = 0;
  ...
  testWidgets('额外预览图接口失败显示具体路由且不暴露异常类型', (tester) async {
    final repository = _FakeMediaRepository(
      downloadError: const SourceException(
        '接口不存在（POST http://127.0.0.1:8001/api/movies/id/7/extrafanart/download）',
        statusCode: 404,
      ),
    );
    ...
    expect(
      find.text(
        '获取额外预览图失败：接口不存在（POST http://127.0.0.1:8001/api/movies/id/7/extrafanart/download）',
      ),
      findsOneWidget,
    );
