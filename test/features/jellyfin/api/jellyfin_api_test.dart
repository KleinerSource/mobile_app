...
class _JellyfinTestAdapter implements HttpClientAdapter {
  _JellyfinTestAdapter(this.respond);
...
void main() {
  test('Jellyfin 目录/详情/剧集接口使用根路径与正确的查询参数', () async {
    final adapter = _JellyfinTestAdapter((options) {
    ...
    expect(adapter.requests, <String>[
      'GET http://test/Users/user-1/Views',
      'GET http://test/Users/user-1/Items'
      ...
      'GET http://test/Users/user-1/Items/Latest'
          '?ParentId=lib-1&Limit=16&EnableImages=true',
      'GET http://test/Users/user-1/Items/Resume?MediaTypes=Video&Limit=6',
      'GET http://test/Shows/NextUp?UserId=user-1&Limit=12',
      'GET http://test/Shows/series-1/Seasons?UserId=user-1',
      'GET http://test/Shows/season-1/Episodes'
          '?UserId=user-1&Fields=Overview%2CMediaSources'
          '&SortBy=ParentIndexNumber%2CIndexNumber',
      'GET http://test/Users/user-1/Items/item-1',
    ]);
  });

  test('authenticateByName 携带 MediaBrowser Authorization 并解析令牌与用户', () async {
    final adapter = _JellyfinTestAdapter((_) => {
      'AccessToken': 'token-1',
      'ServerId': 'server-1',
      'User': {
        'Id': 'user-1',
    ...
    expect(
      ...
      'POST http://test/Users/AuthenticateByName',
    );
  ...
  test('authenticateByName 拒绝空用户名', () {
    final api = JellyfinApi(Dio(BaseOptions(baseUrl: 'http://test')));
  ...
  test('播放会话上报使用 Sessions/Playing 系列端点与 tick 单位', () async {
    ...
    expect(adapter.requests, <String>[
      'POST http://test/Sessions/Playing',
      'POST http://test/Sessions/Playing/Progress',
      'POST http://test/Sessions/Playing/Stopped',
    ]);
  ...
  test('收藏与已看标记分别使用 FavoriteItems/PlayedItems 端点', () async {
    ...
    expect(adapter.requests, <String>[
      'POST http://test/Users/user-1/FavoriteItems/item-1',
      'DELETE http://test/Users/user-1/FavoriteItems/item-1',
      'POST http://test/Users/user-1/PlayedItems/item-1',
      'DELETE http://test/Users/user-1/PlayedItems/item-1',
    ]);
  });
  ...
  test('直链与图片 URL 拼接 ApiKey 与尺寸参数', () {
    final streamUrl = JellyfinApi.streamUrl(
      baseUrl: 'http://test/',
      itemId: 'item 1',
      mediaSourceId: 'ms-1',
      token: 'token-1',
    ...
    expect(
      streamUrl,
      'http://test/Videos/item%201/stream'
      '?static=true&MediaSourceId=ms-1&ApiKey=token-1',
    );

    final imageUrl = JellyfinApi.imageUrl(
      baseUrl: 'http://test',
      itemId: 'item-1',
      maxWidth: 440,
      token: 'token-1',
  ...
  test('resolveJellyfinUrl 把相对转码地址解析为绝对地址', () {
    expect(
      JellyfinApi.resolveJellyfinUrl('http://test', '/videos/1/master.m3u8'),
    ...
    expect(
      JellyfinApi.resolveJellyfinUrl('http://test', 'https://cdn.example/x.m3u8'),
