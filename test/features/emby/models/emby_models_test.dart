...
void main() {
  test('EmbyItem.fromJson 解析电影条目的核心字段', () {
  ...
  test('EmbyItem.fromJson 解析剧集分集字段', () {
    final item = EmbyItem.fromJson(const {
      'Id': 'ep-1',
      'Name': '第一集',
      'Type': 'Episode',
  ...
  test('EmbyItemPage.fromJson 过滤无 ID 条目并计算 hasMore', () {
    final page = EmbyItemPage.fromJson(const {
      'Items': [
    ...
    final lastPage = EmbyItemPage.fromJson(const {
      'Items': [
  ...
  test('EmbyAuthResult.fromJson 解析认证响应', () {
    final result = EmbyAuthResult.fromJson(const {
      'AccessToken': 'token-1',
  ...
  test('tick 与秒互转保持 100ns 单位语义', () {
    expect(embyTicksToSeconds(10000000), 1);
    expect(embyTicksToSeconds(null), 0);
    expect(embyTicksToSeconds(-5), 0);
    expect(secondsToEmbyTicks(90), 900000000);
    expect(embyTicksToSeconds(secondsToEmbyTicks(1234)), 1234);
  });
}
