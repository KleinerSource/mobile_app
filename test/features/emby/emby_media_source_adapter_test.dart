...
class _EmbyAdapter implements HttpClientAdapter {
  _EmbyAdapter(this.respond);
...
void main() {
  ...
  test('descriptor 声明 emby 来源与 catalog/details/playback 能力', () {
    final adapter = buildAdapter(_EmbyAdapter((_) => {}));
    expect(adapter.descriptor.id.value, 'emby');
  ...
  test('listMovies 把 MediaQuery 映射为 Emby 分页查询', () async {
    final httpAdapter = _EmbyAdapter(
      (options) => {
        'Items': [
          {
            'Id': 'item-1',
    ...
    expect(page.items.single.poster, contains('api_key=token-1'));
  ...
  test('getMovie 返回 MediaDetails 并保留 EmbyItem payload', () async {
    ...
    final details = await adapter.getMovie(
      const MediaRef(sourceId: _embySourceId, value: 'item-1'),
    );
    ...
    expect(details.payload, isA<EmbyItem>());
  });

  test('resolvePlayback 默认返回 static 直链并携带恢复位置', () async {
    final httpAdapter = _EmbyAdapter((options) {
    ...
    expect(descriptor.uri.toString(), contains('/emby/Videos/item-1/stream'));
    ...
    expect(
      ...
      contains('GET http://test/emby/Users/user-1/Items/item-1'),
    );
    expect(
      httpAdapter.requests,
      contains(
            ...
            'MediaSources': [
              {
                ...
                'TranscodingUrl': '/emby/videos/item-1/master.m3u8?VideoCodec=h264',
              },
            ],
          };
        }
  ...
  test('imageUrl 拼接 api_key 供图片内核直连', () async {
    final adapter = buildAdapter(_EmbyAdapter((_) => {}));
