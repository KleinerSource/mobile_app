...
void main() {
  test('功能提交递增第二位并重置第三位', () {
    expect(versionBumpForCommit('feat: 添加演员关联管理'), VersionBump.feature);
  ...
  test('优化、改进和修复提交只递增第三位', () {
    expect(versionBumpForCommit('fix: 修复播放器进度同步'), VersionBump.bugFix);
    expect(nextAppVersion('0.2.0+126', VersionBump.bugFix), '0.2.1+127');
    expect(shouldBumpPatchForCommit('refactor: 统一设置页样式'), isTrue);
    expect(versionBumpForCommit('优化: 调整预览图灯箱交互'), VersionBump.bugFix);
  ...
  test('编译失败修复只递增 build 版本', () {
    ...
    expect(nextAppVersion('0.2.1+127', VersionBump.buildOnly), '0.2.1+128');
    expect(
